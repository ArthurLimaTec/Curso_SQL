...
'Z:\REL_LOCADORA.csv'	<---- ONDE O ARQUIVO SERÁ GUARDADO
DELIMITER ';'
CSV HEADER;
...
/*CRIANDO SEQUENCES = AO INDENTITY OU AUTO_INCREMENT FORA DA TABELA
					SERVE PARA IDENTIFICAR CADA NOVA INSERÇÃO COMO ÚNICA, PORÉM
					POR FORA DA TABELA.
					NÃO É COMUM, MAS É UMA OPÇÃO.

*/

CREATE SEQUENCE SEQ_LOCACAO;

NEXTVAL('SEQ_LOCACAO'); /*ESTE É O FORMATO PARA INSERIR EM 'SEQUENCES', OBSERVANDO QUE
							ISTO ESTÁ SENDO INSERIDO NO LUGAR DO ID*/
...
/*RECRIAREMOS A REL_LOCADORA, COLOCANDO O ID QUE NÃO COLOCAMOS ANTES*/
SELECT * FROM REL_LOCADORA;
...
/*PARA SABER SE ESTÁ SINCRONIZADO: OS NÚMEROS DEVEM SER IGUAIS*/
SELECT MAX(IDLOCACAO) AS RELATORIO, (SELECT MAX(IDLOCACAO) FROM LOCACAO) AS LOCACAO
FROM RELATORIO_LOCADORA;

/*VENDO QUAIS NÃO ESTÃO SINCRONIZADOS, OU SEJA, TEM EM UMA MAS NÃO NA OUTRA: NESTE CASO, AGORA, NENHUM!*/
SELECT L.IDLOCACAO, F.NOME AS FILME, G.NOME AS GENERO, L.DATA AS DATA, L.DIAS AS DIAS, L.MIDIA AS MIDIA
FROM GENERO G
INNER JOIN FILME F
ON G.IDGENERO = F.ID_GENERO
...
-----------------------------
/*DEIXANDO ESSE PROCEDIMENTO AUTOMATICO POR MEIO DE UMA TRIGGER:*/
...
/*ESSA TRIGGER FARÁ COM QUE A CADA INSERÇÃO EM LOCAÇÃO SEJA EXECUTADO O PROCEDIMENTO DE ATUALIZAÇÃO
CRIADO 'ATUALIZA_REL()*/
...
/*ISSO SERVIRÁ PARA QUE O RELATÓRIO CONTINUE SINCRONIZADO, POIS ALGO FOI DELETADO DO QUE
JÁ ESTAVA NO RELATÓRIO*/
